...
async function sendWithDedicatedSender(
  ...
  let lienId: string;
  try {
    ({ lien, id: lienId } = await lienInvitation(adminClient, person.email));
  } catch (e) {
  ...
  const text = [
    ...
    "S'il a expiré, la page vous permettra d'en recevoir un nouveau.",
    "",
    `— ${sender.from_name}`,
  ].join("\n");
  ...
  try {
    ...
    await apresEnvoi(adminClient, person.email, lienId, false);
    return {
      ok: false,
  ...
  await apresEnvoi(adminClient, person.email, lienId, true);
  return { ok: true };
}
